...
- **Owner directives are law.** Capture and relay the owner's words in `>` blockquotes with context and analysis in adjacent normal prose (`flow-common` User inputs in context). Group related excerpts under meaningful headings; derived action items do not replace that record. Verify the directive in context was applied. When implementation diverges from an explicit owner choice, escalate — never silently accept.
