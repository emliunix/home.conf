...
**Note:** Use `uv run` for local scripts, `uvx` for installed packages.
...
- Update shebang to `#!/usr/bin/env -S uv run --quiet`
...
2. Run: `uvx --with requests python script.py`
...
3. **Use `--quiet` flag** in shebang to reduce uvx output noise
...
6. **Use `uvx --with`** for quick experiments or external scripts
...
**Module not found during execution:** Use `uvx --with package-name python script.py` for quick fix, or add to inline dependencies for permanent solution
