# uvx Python Script Guide

## Overview

`uvx` (from the `uv` tool) allows running Python scripts with automatic dependency management, avoiding system Python package pollution and virtual environment setup.
...
### Approach 2: `uvx --with` - For Old-Style Scripts

Run existing scripts without modifying them:

```bash
uvx --with pyyaml --with requests python script.py
```
...
# Solution:
uvx --with pyyaml python script.py

# Multiple missing modules:
uvx --with pyyaml --with requests --with pandas python script.py
```
...
1. **Shebang line:** `#!/usr/bin/env -S uv run --quiet`
   - Makes script directly executable
   - `--quiet` suppresses uv output
...
3. **Use `--quiet` flag** in shebang to reduce noise
...
6. **Use `uvx --with`** for quick experiments or third-party scripts
