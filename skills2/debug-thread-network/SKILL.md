...
Example:
```bash
scripts/test_thread_connectivity.sh 192.168.1.100
```
...
Or pipe from API:
```bash
curl -s http://192.168.1.100/diagnostics | \
  jq -r '.[0].NetworkData' | \
...
scripts/query_otbr_api.sh 192.168.1.100
...
OMR_ADDR=$(curl -s http://192.168.1.100/diagnostics | \
